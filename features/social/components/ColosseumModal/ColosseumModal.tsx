  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div
        role="dialog"
        aria-modal="true"
        aria-label="The Colosseum Opens"
        className="relative w-full max-w-lg p-8 overflow-hidden rounded-2xl bg-cc-bg-card/80 border border-amber-500/30 shadow-[0_0_50px_rgba(245,158,11,0.15)] backdrop-blur-xl"
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-cc-text-secondary mt-2 text-sm">{eventData.message}</p>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 mb-8">
          ...
          <button 
            ...
            className={`flex-1 p-4 rounded-xl border transition-all duration-200 ${
              bettingOn === eventData.white.id 
                ...
                : 'bg-cc-bg-sidebar/50 border-cc-border hover:border-cc-border-light'
            }`}
          >
            <div className="text-xl font-bold text-cc-text-primary">{eventData.white.name}</div>
            <div className="text-sm font-medium text-amber-500 mt-1">
              {eventData.odds[eventData.white.id]?.toFixed(2)}x Payout
            </div>
            <div className="text-xs text-cc-text-muted mt-2">{eventData.white.rating} ELO</div>
          </button>

          <div className="text-cc-text-muted font-bold italic">VS</div>
          ...
          >
            <div className="text-xl font-bold text-cc-text-primary">{eventData.black.name}</div>
            <div className="text-sm font-medium text-amber-500 mt-1">
              {eventData.odds[eventData.black.id]?.toFixed(2)}x Payout
            </div>
            <div className="text-xs text-cc-text-muted mt-2">{eventData.black.rating} ELO</div>
          </button>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-cc-text-secondary uppercase tracking-wider mb-2">Wager Amount (Gold)</label>
            <input
              type="number"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className="w-full bg-cc-bg-input/50 border border-cc-border rounded-lg px-4 py-3 text-cc-text-primary placeholder-cc-text-muted focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all"
          ...
          <div className="flex gap-3 pt-4">
            <button
              ...
              className="flex-1 px-4 py-3 rounded-lg font-medium text-cc-text-secondary bg-cc-bg-sidebar hover:bg-cc-bg-hover transition-colors"
            >
              Dismiss
            </button>
            <button 
